...
from knowledge.processor.query_processor.nodes.image_understanding_node import ImageUnderstandingNode
...
def create_query_graph() -> CompiledStateGraph:
    ...
    # 2. 实例化节点
    nodes = {
        "image_understanding_node": ImageUnderstandingNode(),
        "item_name_confirmed_node": ItemNameConfirmedNode(),
        "multi_search": lambda x: {**x, "multi_search_start": "[timestamp]"},  # 虚拟节点
    ...
    # 4. 设置入口点（图片理解节点；无图时透传，不影响纯文本流程）
    workflow.set_entry_point("image_understanding_node")

    # 4.1 图片理解 → 商品名确认
    workflow.add_edge("image_understanding_node", "item_name_confirmed_node")
