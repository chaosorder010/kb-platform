...
def import_router(state: ImportGraphState):
    ...
    if state.get('is_pdf_read_enabled'):
        print(f"[Routing Decision] pdf pattern -> pdf_to_md_node")
        return "pdf_to_md_node"
    if state.get('is_md_read_enabled'):
        print(f"[Routing Decision] md pattern -> md_to_img_node")
        return "md_to_img_node"
    print(f"[Routing Decision] No matching file type")
    return END
...
def import_graph() -> CompiledStateGraph:
    ...
    # 3. 定义其它节点名和节点实例的映射表
    node_name_obj = {
        "entry_node": EntryNode(), #入边：[]; 出边["条件边 -> pdf_to_md_node / md_to_img_node / END"]
        "pdf_to_md_node": PdfToMdNode(), #入边:["entry_node(条件)"]; 出边["md_to_img_node"]
        "md_to_img_node": MarkDownToImgNode(), # 入边: ["pdf_to_md_node", "entry_node(条件)"]; 出边: ["document_split_node"]
        "document_split_node": DocumentSplitNode(), # 入边: ["md_to_img_node"] ; 出边:["item_name_reconition_node"]
        "item_name_recognition_node": ItemNameRecognitionNode(),# 入边: ["document_split_node"]; 出边: ["embeding_chunks_node"]
        "embedding_chunks_node": EmbeddingChunksNode(), # 入边:["item_name_recognition_node"] 出边: ["import_milvus_node"]
        "import_milvus_node": ImportMilvusNode() # 入边:["embding_chunks_node"], [END]
    }
