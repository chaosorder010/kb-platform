...
> 本文档详细介绍掌柜问数项目的环境配置与中间件服务部署，包括 Python 环境搭建、Docker 安装、各中间件部署配置及验证方法。
...
# 如果安装 PyTorch单独安装
pip install torch torchvision -i https://download.pytorch.org/whl/cu128
```
...
不用通过配置文件配置，参考**反向SSH 隧道加速**，可以解决一切。
...
# 连接测试（Python）
python -c "from pymilvus import connections; connections.connect(uri='http://localhost:19530'); print('Connected!')"
```

#### 4.1.3 Attu 控制台访问

- 地址：http://localhost:7000
- 默认连接 Milvus Standalone
...
```bash
# 通过 Web 控制台创建
# 访问 http://localhost:9001 → Buckets → Create Bucket → 输入 "knowledge-base"
```
...
def test_milvus():
    ...
    try:
        ...
        connections.connect(
            alias="default",
            uri=os.getenv("MILVUS_URL", "http://192.168.200.130:19530")
        )
...
def test_mongodb():
    ...
    try:
        from pymongo import MongoClient
        client = MongoClient(
            os.getenv("MONGO_URL", "mongodb://192.168.200.130:27017"),
...
def test_minio():
    ...
    try:
        ...
        client = Minio(
            os.getenv("MINIO_ENDPOINT", "192.168.200.130:9000"),
...
| 统一 API | 8000 | HTTP | knowledge/api/app_main.py（/api/*） |
| 前端开发服 | 5173 | HTTP | frontend/（Vite） |
